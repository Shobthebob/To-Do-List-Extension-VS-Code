...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveTasks() {
        vscode.postMessage({ command: 'saveTasks', tasks: tasks });
    }
    ...
    // New function to recreate tasks from saved state
    ...
    function createTaskElement(task) {
        ...
        taskItem.innerHTML = `
      <input type="checkbox" class="done-chkbx" title="Mark done" ${task.isDone ? 'checked' : ''}>
      <span class="task-txt">${task.text}</span>
      <div class="btns">
    ...
    `;
        ...
        if (task.isDone) {
            toggleTaskDone(taskItem.querySelector('.done-chkbx'), taskItem, false);
        }
    ...
    function toggleTaskDone(checkbox, taskItem, updateState = true) {
    ...
    function unpinTask(taskItem) {
        ...
        if (task.isPinned) {
            ...
            // Move the task to its original position based on insertion order
            todoList.appendChild(taskItem);
        }
        saveTasks();
    }
    ...
    function editTask(taskItem) {
        ...
        function finishEditing() {
            ...
            task.text = taskSpan.textContent;
            saveTasks();
        }
